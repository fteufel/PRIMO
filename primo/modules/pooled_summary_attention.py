...
class PooledSummaryVectorAttentionLayer(nn.Module):
    """
    ...
    def __init__(
        ...
        use_rotary_embeddings: bool = False,
        dropout: float = 0,
        num_pooled_positions: int = 1,
        pool_update_method: str = "pool_to_pool",
    ):
        super().__init__()
        self.embed_dim = embed_dim
        ...
        self.num_pooled_positions = num_pooled_positions
        if pool_update_method not in ["pool_to_seq", "pool_to_pool"]:
            raise ValueError(f"Invalid pool_update_method {pool_update_method}")
        self.pool_update_method = pool_update_method
        ...
        self.self_attn_set = SetMultiheadAttention(
            ...
            add_zero_attn=False,
            use_rotary_embeddings=self.use_rotary_embeddings,
            all_seq_attn=False # performs flattening of key and value sets - not needed when the pooling does the crosstalk
        )
    ...
    def forward(
        ...
        # extract the pooling positions (at the start of the sequence) and combine
        x_pooled = x[:, :, :self.num_pooled_positions, :] # B, N, num_pooled_positions, H
        x_pooled = x_pooled.view(B, N * self.num_pooled_positions, H)

        # approach 1. each sequence does cross attention with pooled positions.
        if self.pool_update_method == "pool_to_seq":
        ...
        # approach 2. pooled positions do self attention, then the pool positions in the original sequences
        elif self.pool_update_method == "pool_to_pool":
            x_pooled, attn = self.self_attn_set(
                query=x_pooled.unsqueeze(1), # dummy head dim
                key=x_pooled.unsqueeze(1),
                value=x_pooled.unsqueeze(1),
                key_padding_mask=None,
                attn_mask=None,
            )
            # the pooled positions in the original sequences are updated with the pooled positions
            x[:, :, :self.num_pooled_positions, :] = x_pooled.squeeze(1).view(B, N, self.num_pooled_positions, H)
